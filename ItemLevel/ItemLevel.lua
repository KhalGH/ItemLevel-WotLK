...
function addon:ADDON_LOADED(arg1)
    if (arg1 == addonName) then
        ...
        panel:SetScript("OnShow", function(this)
            ...
            t2:SetFormattedText("Displays ilvl and border color on the character and inspect panels.\nAllows you to choose between original and transmog icons.\n\nAuthor: |cffc41f3bKhal|r\nVersion: 1.0")
...
function addon:ConfigUI()
    if not ConfigUIFrame then
        local f = CreateFrame("Frame", "ConfigUIFrame", UIParent, "UIPanelDialogTemplate")
        f:SetSize(300, 520)
        f:SetPoint("CENTER")
        ...
        closeButton:SetScript("OnClick", function()
            ...
            f:SetScale(1) -- Reset frame scale
        end)
        ...
        f.title:SetText("ItemLevel Settings")  
        -- Dropdown to change the ilvl text font type
        local FontDropdown = CreateFrame("Frame", "ConfigFrameFontDropdown", f, "UIDropDownMenuTemplate")
        ...
        local sizeSlider = CreateFrame("Slider", "ConfigFrameSizeSlider", f, "OptionsSliderTemplate")
        ...
        local AnchorDropdown = CreateFrame("Frame", "ConfigFrameAnchorDropdown", f, "UIDropDownMenuTemplate")
        ...
        local rgbCheckbox = CreateFrame("CheckButton", "ConfigFrameRGBCheckbox", f, "UICheckButtonTemplate")
        ...
        rgbCheckbox.text:SetText("Use Quality Color Text")
        ...
        local borderSlider = CreateFrame("Slider", "ConfigFrameBorderSlider", f, "OptionsSliderTemplate")
        ...
        borderSlider.tooltipText = "Adjust Border Alpha"
        ...
        borderLabel:SetText("Border Alpha")
        ...
        local avgCheckbox = CreateFrame("CheckButton", "ConfigFrameAVGCheckbox", f, "UICheckButtonTemplate")
        ...
        avgCheckbox.text:SetText("Show Average ItemLevel")
        ...
        local iconCheckbox = CreateFrame("CheckButton", "ConfigFrameIconCheckbox", f, "UICheckButtonTemplate")
        ...
        iconCheckbox.text:SetText("Show Transmog Icons")
        ...
        -- Reset Button
        local resetButton = CreateFrame("Button", "ResetConfigButton", f, "UIPanelButtonTemplate")
        ...
        resetButton:SetPoint("BOTTOM", ConfigUIFrame, "BOTTOM", 0, 15)
        ...
        f:SetScript("OnMouseWheel", function(self, delta)
            ...
            if delta > 0 then
                self:SetScale(math.min(scale + 0.1, 1.333))
            else
                self:SetScale(math.max(scale - 0.1, 0.666))
            end
        end)
        f:Show()
    end
    -- Reset frame position to center when closed
    if not ConfigUIFrame:IsShown() then
...
function addon:UpdateConfigUI()
    if not ConfigUIFrame or not ConfigUIFrame:IsShown() then return end
    if _G["ConfigFrameSizeSlider"] then _G["ConfigFrameSizeSlider"]:SetValue(ilvlConfig.size) end
    if _G["ConfigFrameAnchorDropdown"] then UIDropDownMenu_SetText(_G["ConfigFrameAnchorDropdown"], ilvlConfig.anchor) end
    if _G["ConfigFrameFontDropdown"] then
        UIDropDownMenu_SetText(_G["ConfigFrameFontDropdown"], ilvlConfig.font)
        _G["ConfigFrameFontDropdownText"]:SetFont(FontMapping[ilvlConfig.font], 12, "OUTLINE")
    end
    if _G["ConfigFrameRGBCheckbox"] then _G["ConfigFrameRGBCheckbox"]:SetChecked(ilvlConfig.rgb == 1) end
    if _G["ConfigFrameBorderSlider"] then _G["ConfigFrameBorderSlider"]:SetValue(ilvlConfig.border * 10) end
    if _G["ConfigFrameAVGCheckbox"] then _G["ConfigFrameAVGCheckbox"]:SetChecked(ilvlConfig.avg == 1) end
    if _G["ConfigFrameIconCheckbox"] then _G["ConfigFrameIconCheckbox"]:SetChecked(ilvlConfig.icon == 1) end
end
...
function addon:UpdateSlots(unit, frameType)
    ...
    _G["Ileveltxt"]:Show()
    local ItemCount = 0; local TotalIlvl = 0; local AvgIlvl = 0; local ilvltemp = 0;
      for k = 1,18 do																	
        if not (k == 4) then														
            if (GetInventoryItemLink("player", k)) then                             
                _, _, _, ilvltemp  = GetItemInfo(GetInventoryItemLink("player", k))  
                if ilvltemp then
    ...
    if (AvgIlvl >= 219) then
        _G["Ilevelnr"]:SetTextColor(0.69, 0.28, 0.97, 1)								
    elseif (AvgIlvl >= 200) then
        _G["Ilevelnr"]:SetTextColor(0, 0.5, 1, 1)												
    elseif (AvgIlvl >= 180) then
        _G["Ilevelnr"]:SetTextColor(0.12, 1, 0, 1)
    else
        _G["Ilevelnr"]:SetTextColor(1, 1, 1, 1)											
    end
    _G["Ilevelnr"]:Show()
